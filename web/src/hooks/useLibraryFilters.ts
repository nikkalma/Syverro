...
export function useLibraryFilters(books: EnrichedBook[]) {
  ...
  const allCenturies = Array.from(
    new Set(
      books
        .map(book => getCentury(book.originalYear))
        .filter(century => century !== 'Неизвестно')
    )
  );
  ...
  const filteredBooks = books.filter(book => {
    ...
    if (
      ...
      !book.themes?.some(theme => selectedThemes.includes(theme))
    ) {
      return false;
    }
    ...
    if (
      ...
      !selectedCountries.includes(book.authorCountry || '')
    ) {
      return false;
    }
    ...
    if (selectedCenturies.length > 0) {
      ...
      if (!selectedCenturies.includes(bookCentury)) {
        return false;
      }
    }
