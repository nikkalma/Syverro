...
export default function Fill() {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
    <EditorSectionCard title="Fill" description="Run one grounded research domain at a time. Every result remains a proposal until a human reviews and explicitly applies it.">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
        {DOMAINS.map((domain) => { const state = corpus?.domains[domain.key]; const capable = corpus?.verified_sources.filter((s) => (s.stored_content_capabilities || s.content_capabilities).includes(domain.key.toUpperCase())).length || 0; return <div key={domain.key} style={{ padding: 14, border: '1px solid var(--border-soft)', borderRadius: 9, background: 'var(--surface-hover)' }}>
          ...
          <div style={{ fontSize: 11, color: state?.available ? '#4CAF50' : '#FFA726', marginBottom: 10 }}>{state ? (state.available ? `${capable} verified capable source${capable === 1 ? '' : 's'}` : corpusReasonMessage(state.reason)) : 'Checking verified corpus…'}</div>
          {state?.available ? <button type="button" disabled={!!running} aria-label={`Run ${domain.label} Fill`} onClick={() => run(domain.key)} style={{ padding: '7px 12px', border: 0, borderRadius: 6, background: 'var(--accent)', color: '#fff', opacity: running ? .5 : 1, cursor: running ? 'not-allowed' : 'pointer' }}>{running === domain.key ? 'Running…' : 'Run Fill'}</button> : null}
          {state && !state.available && state.reason ? <details style={{ color: 'var(--text-muted)', fontSize: 10 }}><summary>Technical details</summary><div>Reason code: {state.reason}</div></details> : null}
    ...
    <EditorSectionCard title="Recent SyvAI runs" description="Operational details stay secondary to the editorial result.">
      {runs.slice(0, 10).map((run) => <div key={run.id} style={{ padding: '8px 0', borderBottom: '1px solid var(--border-soft)', fontSize: 12 }}><strong>{run.domain.replace('_', ' ')}</strong> · {run.status} · {run.proposal_count || 0} proposals · {run.source_count} sources <span style={{ color: 'var(--text-muted)' }}>{run.created_at ? new Date(run.created_at).toLocaleString() : ''}</span>{run.error && <div style={{ color: 'var(--error)' }}>{run.error.includes('UNSUPPORTED') || run.error.includes('NO_VERIFIED_SOURCES') || run.error.includes('INSUFFICIENT_CORPUS') ? corpusReasonMessage(run.error) : run.error}</div>}<details style={{ color: 'var(--text-muted)', marginTop: 4 }}><summary>Technical details</summary>{run.provider}{run.model ? ` / ${run.model}` : ''} · {run.total_tokens || 0} tokens · {run.duration_ms ? `${Math.round(run.duration_ms / 1000)}s` : 'duration unavailable'}{run.error ? ` · ${run.error}` : ''}</details></div>)}
