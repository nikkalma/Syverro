...
/**
 * Совместимый тип книги для UI.
 *
 * GlobalBook = данные самой книги.
 * PersonalBook = данные пользователя о книге.
 *
 * UI пока использует объединённый объект,
 * поэтому оставляем этот слой совместимости.
 */
export type Book = GlobalBook &
  Partial<PersonalBook> & {
    review?: string | null;
    readingFormat?: ReadingFormat;
    lastRead?: number | null;
  };
...
export type NewGlobalBook = Omit<
  GlobalBook,
  'id' | 'createdAt' | 'averageRating' | 'totalRatings'
>;
...
export type { GlobalBook, NewGlobalBook, EnrichedBook } from './globalBook';
