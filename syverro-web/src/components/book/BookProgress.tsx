...
export default function BookProgress({
  ...
  const currentPage = personal?.currentPage ?? 0;
    ...
    <div className="bg-[#121C24] border border-[#2A4B60] rounded-2xl p-6 mb-6">
      <div className="flex justify-between items-center mb-3">
        ...
        <span className="text-sm text-[#97A6BA]">
          {currentPage} / {book.totalPages || '?'} стр.
        </span>
      </div>
      ...
      <div className="relative">
        <input
          ...
          value={currentPage}
          onChange={(e) =>
            onUpdateProgress(Number(e.target.value))
          }
      ...
      {personal?.lastRead && (
        <p className="text-xs text-[#5B86A1] mt-3">
          Последнее чтение:{' '}
          {new Date(personal.lastRead).toLocaleDateString('ru-RU')}
        </p>
      )}
    </div>
  );
