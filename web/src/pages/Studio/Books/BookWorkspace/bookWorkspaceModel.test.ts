...
describe('book workspace readiness', () => {
  it('requires only fields editable in the five-section workspace', () => {
    const book = {
      ...
      original_publication_year: 2001, total_pages: 320, description: 'Description', genre_ids: ['g1'],
  ...
  it('maps missing fields to their editable section', () => {
    ...
    expect(getBookReadiness(book).map(({ key, section }) => `${section}:${key}`)).toEqual([
      'identity:title', 'identity:authors', 'identity:cover', 'identity:publication_year', 'identity:pages', 'editorial:description', 'knowledge:genres',
    ]);
  });
});
