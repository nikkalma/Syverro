...
export default function SortModal({ visible, setVisible, sortBy, setSortBy, lang }) {
  const { theme } = useTheme();

  const sortOptions = [
    { key: 'date', label: lang.sort?.byDate || 'По дате добавления', icon: '📅' },
    { key: 'title', label: lang.sort?.byTitle || 'По названию', icon: '🔤' },
    { key: 'author', label: lang.sort?.byAuthor || 'По автору', icon: '👤' },
    { key: 'rating', label: lang.sort?.byRating || 'По оценке', icon: '⭐' },
  ];
      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'center', padding: 20 }}>
        <View style={{ backgroundColor: theme.surface, borderRadius: 16, padding: 20 }}>
          <Text style={{ color: theme.textPrimary, fontSize: 18, marginBottom: 16, textAlign: 'center' }}>
            {lang.sort?.title || 'Сортировать по'}
          </Text>
          ...
          <TouchableOpacity 
            ...
            activeOpacity={0.4}
          >
            <Text style={{ color: theme.textPrimary, textAlign: 'center' }}>{lang.buttons?.cancel || 'Отмена'}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
